...
# Multi-Route Price Calculator Endpoint
@api_router.post("/calculate-route-options", response_model=dict)
async def calculate_route_options(request: dict):
    """Calculate both fastest and shortest route options for customer choice"""
    try:
        ...
        if not origin or not destination:
            raise HTTPException(
                status_code=400,
                detail="Start- und Zieladresse sind erforderlich"
            )
